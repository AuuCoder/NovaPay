...
export function MerchantRegistryPurchaseFinalizer(props: {
  slug?: string;
  locale: "zh" | "en";
}) {
  ...
  useEffect(() => {
    ...
    if (!registryPluginSlug || !registryOrderId || (props.slug && registryPluginSlug !== props.slug)) {
      return;
    }
    ...
    async function run() {
      try {
        const response = await fetch("/merchant/plugins/finalize-registry-purchase", {
          method: "POST",
          headers: {
        ...
        setMessage(
          props.locale === "en"
            ? "Registry purchase completed and merchant license recorded."
            : "Registry 购买已完成，商户许可证已自动登记。",
        );
