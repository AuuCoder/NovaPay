...
import { getFinanceWorkerConfig, runFinanceMaintenance } from "../lib/finance/settlements";
...
async function main() {
  ...
  const config = await getFinanceWorkerConfig();
  const intervalMs = parseNumberOption(args, "--interval-ms") ?? config.intervalMs;
  const holdDays = parseNumberOption(args, "--hold-days") ?? config.settlementHoldDays;
  ...
  const shutdown = (signal: string) => {
    ...
    shuttingDown = true;
    console.log(`[finance-worker] received ${signal}, waiting for current loop to finish...`);
  };
  ...
  console.log(
    `[finance-worker] started with intervalMs=${intervalMs} holdDays=${holdDays} mode=${
      runOnce ? "once" : "daemon"
    }`,
  );
...
main().catch(async (error) => {
  console.error("[finance-worker] fatal error");
  console.error(error);
