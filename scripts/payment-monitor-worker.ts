...
import {
  getPaymentMonitorWorkerConfig,
  runDuePaymentOrderMonitorDispatches,
} from "../lib/orders/payment-monitor";
...
async function main() {
  ...
  const config = await getPaymentMonitorWorkerConfig();
  ...
  const shutdown = (signal: string) => {
    ...
    shuttingDown = true;
    console.log(`[payment-monitor-worker] received ${signal}, waiting for current loop to finish...`);
  };
  ...
  console.log(
    `[payment-monitor-worker] started with limit=${limit} intervalMs=${intervalMs} mode=${
      runOnce ? "once" : "daemon"
    }`,
  );
  ...
  do {
    ...
    const result = await runDuePaymentOrderMonitorDispatches(limit);
    console.log(
      JSON.stringify(
        {
...
main().catch(async (error) => {
  console.error("[payment-monitor-worker] fatal error");
  console.error(error);
